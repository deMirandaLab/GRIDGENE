...
class GetMasks:
    ...
    def filter_mask_by_area(self, mask, min_area):
        ...
        """
        Filter the mask by removing connected components with an area smaller than min_area,
        while preserving the original labels of the remaining components.

        :param mask: Input labeled mask to be filtered.
        :param min_area: Minimum area threshold for connected components to be retained.
        :return: Filtered mask with only the valid connected components, retaining original labels.
        """
        ...
        # Create a boolean array to identify labels to keep (excluding background label 0)
        labels_to_keep = unique_labels[counts >= min_area]
        labels_to_keep = labels_to_keep[labels_to_keep != 0]

        # Create a new mask to store the filtered components
        filtered_mask = np.zeros_like(mask, dtype=np.int32)

        # Iterate over labels to keep and assign them to the filtered mask
        for label in labels_to_keep:
            filtered_mask[mask == label] = label

        return filtered_mask
    # added this function to get the contours from the mask
    def create_mask(self, contours):
    ...
    def apply_morphology(self, mask, operation="open", kernel_size=3):
        """Applies morphological operations to smooth or refine masks."""
        """
         Applies morphological operations to refine binary masks.

         Morphological operations help clean up segmentation masks by removing noise,
         filling holes, or refining object boundaries.

         :param mask: Binary mask to be processed (numpy array).
         :param operation: Type of morphological operation to apply:
             - "open"  -> Removes small noise (erosion followed by dilation).
             - "close" -> Fills small holes (dilation followed by erosion).
             - "erode" -> Shrinks white regions (removes small pixels, useful for noise reduction).
             - "dilate" -> Expands white regions (connects broken parts of objects).
         :param kernel_size: Size of the structuring element (larger values create stronger effects).
        ...
        return mask  # Return original if no valid operation

    def subtract_masks(self, base_mask, *masks):
        """
        Subtracts multiple masks from a base mask.

        :param base_mask: The initial binary mask.
        :param masks: One or more masks to be subtracted from the base.
        :return: The resulting mask after subtraction.
        """
    ...
    # def get_stroma_mask(self, mask_tum, mask_empty):
    #     """Computes the stroma mask as (1 - tumor - empty)."""
    #     mask_stroma = np.ones((self.height, self.width), dtype=np.uint8)
    #     mask_stroma = cv2.subtract(mask_stroma, mask_tum)
    #     mask_stroma = cv2.subtract(mask_stroma, mask_empty)
    #     return mask_stroma
    ...
    #
    #     # Subtract the tumour mask from the empty mask
    #     self.mask_empty = cv2.subtract(self.mask_empty, self.mask_T)
    #
...
class TumBorderAnalysis(GetMasks):
    ...
    #     """
    #     # Step 0: Label the original tumour mask (assumed binary: 0=background, >0=tumour)
    #     num_tumour_labels, labelled_tumour = cv2.connectedComponents(self.mask_T.astype(np.uint8))
    #     labelled_tumour = labelled_tumour.astype(np.uint8)
    #
    #     self.mask_T_labelled = labelled_tumour
    #     # 'assigned' will keep track of all pixels that have already been assigned (tumour + expanded borders)
    #     assigned = self.mask_T.copy()
    #     masks_T_borders = []  # To store each expanded border mask
    #
    ...
    #         # Create an empty mask for the expanded borders (same shape as self.mask_T)
    #         expanded_border_mask = np.zeros_like(self.mask_T, dtype=np.uint8)
    #
    #         # Process each tumour object individually (exclude background label 0)
    #         unique_labels = np.unique(labelled_tumour)
    #         unique_labels = unique_labels[unique_labels != 0]
    #         kernel = np.ones((kernel_size, kernel_size), np.uint8)
    #
    #         dilated_labelled_tumour = cv2.dilate(labelled_tumour, kernel, iterations=1)
    #
    #         # Step 2: Identify the border regions
    #         border_regions = np.where((dilated_labelled_tumour != labelled_tumour)
    #                                   & (labelled_tumour == 0)
    #                                   & (assigned == 0),
    #                                   dilated_labelled_tumour, 0)
    #
    #         # Step 3: Assign labels to the border regions
    #         expanded_border_mask = border_regions.astype(np.uint8)
    #         # Optionally filter border objects by area if filter_area is provided
    #         if filter_area is not None:
    #             self.logger.info(f'Filtering Border mask by area: {filter_area}')
    #             expanded_border_mask = self.filter_mask_by_area(expanded_border_mask, min_area=filter_area)
    #
    #         # Update the binary "assigned" mask (combine current tumour + border)
    #         assigned = cv2.bitwise_or(assigned, (expanded_border_mask > 0).astype(self.mask_T.dtype))
    #         masks_T_borders.append(expanded_border_mask)
    #
    #
    #     # Subtract the combined mask from the original mask stroma
    #     # assigned = assigned.astype(self.mask_S.dtype)
    #     # mask_remaining_S = cv2.subtract(self.mask_S, assigned)
    #     expanded_mask_combined = np.zeros_like(self.mask_T, dtype=np.uint8)
    #     for mask in masks_T_borders:
    #         # expanded_mask_combined = cv2.bitwise_or(expanded_mask_combined, mask)
    #         expanded_mask_combined = cv2.bitwise_or(expanded_mask_combined, mask.astype(np.uint8))
    #
    #     # expanded_mask_combined = expanded_mask_combined.astype(self.mask_S.dtype)
    #     # mask_remaining_S = cv2.subtract(self.mask_S,
    #     #                                 expanded_mask_combined)
    #     mask_remaining_S = cv2.bitwise_and(self.mask_S, cv2.bitwise_not(assigned))
    #
    #     self.mask_T_tborder = self.mask_T
    #     self.mask_S_tborder = mask_remaining_S
    #     self.mask_TB_tborders_labelled = masks_T_borders
    #     self.mask_TB_tborders = [mask > 0 for mask in masks_T_borders]
    #     return


    def get_mask_tumour_border(self, expansions_pixels=[], filter_area=None, restrict_to_stroma=True):
        """
        Expands the tumour border by a specified number of pixels and returns the updated masks.

        :param expansions_pixels: List of integers specifying the number of pixels to expand the tumour border.
        :param filter_area: Minimum area threshold for filtering the expanded tumour border masks. If None, no filtering is applied.
        :param restrict_to_stroma: If True, ensures expansions only occur within the stroma mask.
        :return: None. Updates instance attributes.
        """
        # Step 0: Label the original tumour mask
        num_tumour_labels, labelled_tumour = cv2.connectedComponents(self.mask_T.astype(np.uint8))
        labelled_tumour = labelled_tumour.astype(np.uint8)

        self.mask_T_labelled = labelled_tumour

        # 'assigned' tracks all assigned pixels (tumour + expanded borders)
        assigned = self.mask_T.copy()
        masks_T_borders = []  # Store each expanded border mask

        # Initialize empty mask to track all expansions
        total_expansion = np.zeros_like(self.mask_T, dtype=np.uint8)

        for kernel_size in expansions_pixels:
            kernel = np.ones((kernel_size, kernel_size), np.uint8)

            # Expand tumour mask using dilation
            dilated_tumour = cv2.dilate(self.mask_T, kernel, iterations=1)

            # Get border by subtracting original tumour mask
            expanded_border = cv2.bitwise_xor(dilated_tumour, self.mask_T)

            if restrict_to_stroma:
                # Convert both to uint8 to prevent OpenCV errors
                expanded_border = expanded_border.astype(np.uint8)
                mask_S = self.mask_S.astype(np.uint8)
                # Ensure border is only within stroma
                expanded_border = cv2.bitwise_and(expanded_border, mask_S)

            # Subtract previous expansions to get exclusive mask for current step
            exclusive_border = cv2.bitwise_and(expanded_border, cv2.bitwise_not(total_expansion))

            # Filter by area if required
            if filter_area is not None:
                self.logger.info(f'Filtering border mask by area: {filter_area}')
                exclusive_border = self.filter_mask_by_area(exclusive_border, min_area=filter_area)

            # Update tracking masks
            total_expansion = cv2.bitwise_or(total_expansion.astype(np.uint8), exclusive_border.astype(np.uint8))
            assigned = cv2.bitwise_or(assigned.astype(np.uint8), exclusive_border.astype(np.uint8))
            masks_T_borders.append(exclusive_border)

        # Remove all expansions from the stroma mask
        updated_stroma = cv2.bitwise_and(mask_S, cv2.bitwise_not(total_expansion).astype(np.uint8))

        # Update instance attributes
        self.mask_T_tborder = self.mask_T
        self.mask_S_tborder = updated_stroma
        self.mask_TB_tborders_labelled = masks_T_borders
        self.mask_TB_tborders = [mask > 0 for mask in masks_T_borders]
